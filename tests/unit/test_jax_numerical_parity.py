"""Synthetic module smoke comparisons against JAX AF3 outputs.
...
This validates module wiring and numerical agreement on synthetic inputs with
randomly initialized parameters. These fixtures cannot satisfy representative
or end-to-end acceptance gates.
...
# Tight tolerances for the synthetic module smoke fixtures.
TOLERANCES = {
...
# Synthetic end-to-end smoke RMSD tolerance.
SYNTHETIC_E2E_RMSD_TOLERANCE = 0.5  # Angstrom
...
def _check_synthetic_reference_provenance(
    ref_data: np.lib.npyio.NpzFile,
) -> None:
    """Verify consumers treat these fixtures only as synthetic smoke data."""
    manifest = _load_manifest()
    assert manifest.get("reference_scope") == "synthetic_module_smoke"
    assert manifest.get("representative_certification_eligible") is False
    compliance = manifest.get("compliance", {})
    assert not any(
        value
        for key, value in compliance.items()
        if key.startswith(("sc002_", "sc003_")) and key.endswith("_validated")
    )
    for flag in ("sc002_compliant", "sc003_compliant"):
        if flag in ref_data.keys():
            assert not bool(ref_data[flag]), (
                f"Synthetic smoke fixture must not claim {flag}=True"
            )
    if "representative_certification_eligible" in ref_data.keys():
...
def _check_params_exist(ref_data: np.lib.npyio.NpzFile) -> list[str]:
    ...
    _check_synthetic_reference_provenance(ref_data)
    param_keys = [k for k in ref_data.keys() if k.startswith("params/")]
    assert len(param_keys) > 0, "No params/* keys found in synthetic smoke fixture"
    return param_keys
...
def _load_transformer(transformer, ref_data: np.lib.npyio.NpzFile, prefix: str, name_prefix: str) -> None:
    ...
    # AF3 stores one projection per super-block in a leading stacked dimension.
    # Loading only a single projection would leave most diffusion blocks random.
...
# ============================================================================
# synthetic module smoke: PairFormer Full Parity Tests
# ============================================================================

class TestPairFormerJAXAF3Parity:
    """Full PairFormerIteration numerical parity tests against JAX AF3 (synthetic module smoke)."""
    ...
    def test_pairformer_outputs_match_jax(self, ref_data):
        """Test PairFormer single and pair outputs match JAX AF3 reference (synthetic module smoke)."""
        ...
        np.testing.assert_allclose(
            ...
            err_msg="synthetic module smoke FAILED: PairFormer single output differs from JAX reference",
        )
        ...
        np.testing.assert_allclose(
            ...
            rtol=TOLERANCES["pairformer"]["rtol"],
            atol=TOLERANCES["pairformer"]["atol"],
            err_msg="synthetic module smoke FAILED: PairFormer pair output differs from JAX reference",
        )

        print(f"\n=== synthetic module smoke PairFormer Parity PASSED ===")
    ...
    def test_pairformer_has_params_keys(self, ref_data):
        """Test that reference file has params/* keys (synthetic module smoke requirement)."""
        param_keys = _check_params_exist(ref_data)
        print(f"\n=== synthetic module smoke PairFormer Params Check PASSED ===")
        print(f"  Found {len(param_keys)} param tensors")


# ============================================================================
# synthetic module smoke: Evoformer Stack NUMERICAL Parity Tests
# ============================================================================

class TestEvoformerJAXAF3Parity:
    """Evoformer stack NUMERICAL parity tests against JAX AF3 (synthetic module smoke)."""
    ...
    def test_evoformer_has_params_keys(self, ref_data):
        """Test that Evoformer reference has params/* keys (synthetic module smoke requirement)."""
        param_keys = _check_params_exist(ref_data)
        assert len(param_keys) > 0, "No params found in evoformer_ref.npz"
        print(f"\n=== synthetic module smoke Evoformer Params Check PASSED ===")
        print(f"  Found {len(param_keys)} param tensors")

    def test_evoformer_numerical_parity(self, ref_data):
        """Test Evoformer numerical parity with loaded params (synthetic module smoke).
        ...
        np.testing.assert_allclose(
            ...
            err_msg="synthetic module smoke FAILED: Evoformer single output differs from JAX reference",
        )
        ...
        np.testing.assert_allclose(
            ...
            rtol=TOLERANCES["evoformer"]["rtol"],
            atol=TOLERANCES["evoformer"]["atol"],
            err_msg="synthetic module smoke FAILED: Evoformer pair output differs from JAX reference",
        )

        print(f"\n=== synthetic module smoke Evoformer Numerical Parity PASSED ===")
        print(f"  Single max diff: {np.max(np.abs(mlx_single_out - jax_single_out)):.2e}")
        print(f"  Pair max diff: {np.max(np.abs(mlx_pair_out - jax_pair_out)):.2e}")


# ============================================================================
# synthetic module smoke: Diffusion Step NUMERICAL Parity Tests
# ============================================================================

class TestDiffusionStepJAXAF3Parity:
    """Diffusion head NUMERICAL parity tests using real AF3 DiffusionHead (synthetic module smoke)."""
    ...
    def test_diffusion_has_params_keys(self, ref_data):
        """Test that diffusion reference has params/* keys (synthetic module smoke requirement)."""
        param_keys = _check_params_exist(ref_data)
        print(f"\n=== synthetic module smoke Diffusion Params Check PASSED ===")
        print(f"  Found {len(param_keys)} param tensors")

    def test_diffusion_head_numerical_parity(self, ref_data):
        """Test MLX DiffusionHead numerical parity vs JAX AF3 reference (synthetic module smoke)."""
        ...
        np.testing.assert_allclose(
            ...
            err_msg="synthetic module smoke FAILED: DiffusionHead output differs from JAX reference",
        )

        print(f"\n=== synthetic module smoke DiffusionHead Numerical Parity PASSED ===")
        print(f"  Max diff: {np.max(np.abs(np.array(denoised) - jax_out)):.2e}")


# ============================================================================
# synthetic module smoke: Confidence Head NUMERICAL Parity Tests
# ============================================================================

class TestConfidenceHeadJAXAF3Parity:
    """Confidence head NUMERICAL parity tests using real AF3 ConfidenceHead (synthetic module smoke)."""
    ...
    def test_confidence_has_params_keys(self, ref_data):
        """Test that confidence reference has params/* keys (synthetic module smoke requirement)."""
        param_keys = _check_params_exist(ref_data)
        print(f"\n=== synthetic module smoke Confidence Params Check PASSED ===")
        print(f"  Found {len(param_keys)} param tensors")

    def test_confidence_head_numerical_parity(self, ref_data):
        """Test MLX ConfidenceHead numerical parity vs JAX AF3 reference (synthetic module smoke)."""
        ...
        assert plddt_err < TOLERANCES["confidence"]["relative_error"], (
            f"synthetic module smoke FAILED: pLDDT relative error {plddt_err:.4f} exceeds tolerance"
        )
        assert pae_err < TOLERANCES["confidence"]["relative_error"], (
            f"synthetic module smoke FAILED: PAE relative error {pae_err:.4f} exceeds tolerance"
        )
        assert pde_err < TOLERANCES["confidence"]["relative_error"], (
            f"synthetic module smoke FAILED: PDE relative error {pde_err:.4f} exceeds tolerance"
        )

        print(f"\n=== synthetic module smoke ConfidenceHead Numerical Parity PASSED ===")
...
# ============================================================================
# synthetic end-to-end smoke: End-to-End RMSD Test
# ============================================================================

class TestSyntheticEndToEndRMSD:
    """Synthetic end-to-end smoke parity against JAX AF3 model outputs."""
    ...
    def test_end_to_end_has_params_keys(self, ref_data):
        """Test that end-to-end reference has params/* keys (synthetic end-to-end smoke requirement)."""
        param_keys = _check_params_exist(ref_data)
        print(f"\n=== synthetic end-to-end smoke End-to-End Params Check PASSED ===")
        print(f"  Found {len(param_keys)} param tensors")

    def test_end_to_end_rmsd_parity(self, ref_data):
        """Test synthetic end-to-end smoke: MLX Model RMSD vs JAX AF3 output."""
        ...
        assert rmsd < SYNTHETIC_E2E_RMSD_TOLERANCE, (
            f"synthetic end-to-end smoke FAILED: RMSD {rmsd:.4f}Å exceeds {SYNTHETIC_E2E_RMSD_TOLERANCE:.2f}Å"
        )

        print(f"\n=== synthetic end-to-end smoke End-to-End RMSD Parity PASSED ===")
