...
You can provide input JSON to the MLX CLI or the API:

-   CLI: `run_alphafold_mlx.py --input <input.json> --output_dir <output_dir>`
-   API: submit JSON payloads to `POST /api/jobs` or `POST /api/validate`
...
We provide a converter in `run_alphafold_mlx.py` which automatically detects the
...
list, then this is considered incorrect and `run_alphafold_mlx.py` will raise an
error.
...
The top-level `version` field (for the `alphafold3` dialect) can be `1`, `2`,
`3`, or `4`. The following features were added in each version:
...
[`jq`](https://github.com/jqlang/jq) command-line tool (`brew install jq` on macOS):
